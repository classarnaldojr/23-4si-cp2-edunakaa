...
def detect_jogada(frame):
    ...
    res_pedra = cv2.matchTemplate(gray, pedra_template_esquerda, cv2.TM_CCOEFF_NORMED)
    res_papel = cv2.matchTemplate(gray, papel_template_esquerda, cv2.TM_CCOEFF_NORMED)
    res_tesoura = cv2.matchTemplate(gray, tesoura_template_esquerda, cv2.TM_CCOEFF_NORMED)
...
def detect_jogada_2(frame):
    ...
    res_pedra = cv2.matchTemplate(gray, pedra_template_direita, cv2.TM_CCOEFF_NORMED)
    res_papel = cv2.matchTemplate(gray, papel_template_direita, cv2.TM_CCOEFF_NORMED)
    res_tesoura = cv2.matchTemplate(gray, tesoura_template_direita, cv2.TM_CCOEFF_NORMED)
...
while True:
    ...
    if jogada_1 is not None:
        cv2.putText(frame, jogada_1, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)

    if jogada_2 is not None:
        cv2.putText(frame, jogada_2, (500, 49), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
...
#mostra quem ganhou a rodada na tela
    if result == 0:
        cv2.putText(frame, "Empate!", (meio_tela, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
    elif result == -1:
        cv2.putText(frame, "Jogador Esquerda ganhou!", (meio_tela, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
    elif result == 1:
        cv2.putText(frame, "Jogador Direita ganhou!", (meio_tela, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)

    cv2.putText(frame, f"Placar: (Jogador da Esquerda) {placar_1} - {placar_2} (Jogador da Direita)", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
